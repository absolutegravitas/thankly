...
import { isWithinExpirationDate } from '@/utilities/isWithinExperationDate'
...
export function PayloadAdapter(payload: Payload, options: PayloadAdapterOptions = {}): Adapter {
  ...
  const userCollectionName =
    options.collectionNames?.users || PAYLOAD_ADAPTER_CONFIG.collectionNames.users
  const sessionCollectionName =
    options.collectionNames!.sessions || PAYLOAD_ADAPTER_CONFIG.collectionNames.sessions
  ...
  return {
    ...
    async createSession({ sessionToken, userId, expires }) {
      ...
      return {
        ...
        userId: sessionUserId as unknown as string,
    ...
    async updateSession({ sessionToken, expires }) {
      ...
      const sessionUserId =
        typeof updatedSession?.user === 'string' || typeof updatedSession?.user === 'number'
          ? updatedSession.user
