...
import { imageHero } from './media/image-hero'
import { NextRequest, NextResponse } from 'next/server'
// import { useAuth } from '@/app/(app)/_providers/Auth'
import { fetchMe } from '@/app/(app)/_queries/fetchMe'
...
//export const GET: PayloadHandler = async (req, res): Promise<void> => {
...
export async function GET(request: NextRequest) {
  ...
  const [imageHeroDoc] = await Promise.all([
    await payload.create({
      collection: 'media',
      filePath: path.resolve(__dirname, '../../../../../src/app/(payload)/seed/media/image-hero.jpg'),
      data: imageHero,
    }),
    // await payload.create({
    //   collection: 'media',
  ...
  const imageHeroID = imageHeroDoc.id;
  ...
  await payload.create({
    ...
    data: JSON.parse(
      JSON.stringify(home)
        .replace(/"\{\{IMAGE_HERO\}\}"/g, imageHeroID)
