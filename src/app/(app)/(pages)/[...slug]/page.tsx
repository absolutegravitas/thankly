...
export const fetchPage = cache(async (slug: string): Promise<Page | null> => {
  const config = await configPromise
  let payload: any = await getPayloadHMR({ config })
  let page = null
...
// Handle the dynamic page rendering
    ...
    return {
      ...
      openGraph: {
        ...
        description:
          'Express your gratitude with Thankly - Your one-stop shop for thoughtful gifts and cards.',
        url: Array.isArray(slug) ? slug.join('/') : '/',
        images: [{ url: `${process.env.NEXT_PUBLIC_SERVER_URL}/images/og-image.png` }],
      },
    }
  }
