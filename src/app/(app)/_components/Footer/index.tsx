...
// import { Text } from '@web/_components/forms/fields/Text'
// import FormComponent from '@web/_components/forms/Form'
// import { validateEmail } from '@web/_components/forms/validations'
...
// import Payload3D from '@web/_components/Payload3D'
// import { DiscordIcon } from '@web/_graphics/DiscordIcon'
import { FacebookIcon } from '@web/_graphics/FacebookIcon'
import { InstagramIcon } from '@web/_graphics/InstagramIcon'
import { ThemeAutoIcon } from '@web/_graphics/ThemeAutoIcon'
import { ThemeDarkIcon } from '@web/_graphics/ThemeDarkIcon'
import { ThemeLightIcon } from '@web/_graphics/ThemeLightIcon'
// import { TwitterIconAlt } from '@web/_graphics/TwitterIconAlt'
// import { YoutubeIcon } from '@web/_graphics/YoutubeIcon'
import { ChevronUpDownIcon } from '@web/_icons/ChevronUpDownIcon'
import { useHeaderObserver } from '@web/_providers/HeaderIntersectionObserver'
import { useThemePreference } from '@web/_providers/Theme'
import { getImplicitPreference, themeLocalStorageKey } from '@web/_providers/Theme/shared'
import { Theme } from '@web/_providers/Theme/types'
import { getCookie } from '@web/_utilities/get-cookie'
        ...
        <div className={[classes.grid, 'grid'].filter(Boolean).join(' ')}>
          {/* <div className={['cols-4 cols-m-8 cols-s-8'].filter(Boolean).join(' ')}>
            ...
            <div className={classes.colItems}>
              {products?.navItems?.map(({ link }: any, index: any) => {
                return (
          ...
          <div className={['cols-4 cols-m-8 cols-s-8'].filter(Boolean).join(' ')}>
            ...
            <div className={classes.colItems}>
              {developers?.navItems?.map(({ link }: any, index: any) => {
                return (
                  <React.Fragment key={index}>
                    <CMSLink className={classes.link} {...link} />
                  </React.Fragment>
          ...
          <div className={['cols-4 cols-m-8 cols-s-8'].filter(Boolean).join(' ')}>
            ...
            <div className={classes.colItems}>
              {company?.navItems?.map(({ link }: any, index: any) => {
                return (
                  <React.Fragment key={index}>
                    <CMSLink className={classes.link} {...link} />
                  </React.Fragment>
